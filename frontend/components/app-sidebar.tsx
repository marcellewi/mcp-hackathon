...
export async function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  const contexts = (await fetch(`${process.env.API_URL}/api/logs`).then((res) => res.json())) as Contexts;
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {contexts.map((context) => (
                <SidebarMenuItem key={context.filename} className="flex items-center justify-between pr-2">
